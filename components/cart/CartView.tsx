...
const [coupon, setCoupon] = useState("");
  ...
  if (cart.length === 0) {
    return (
      <main className="min-h-screen bg-background pb-28" dir="rtl">
        <header className="flex items-center justify-between border-b bg-background/95 px-4 py-4">
        ...
        <div className="flex min-h-[70vh] flex-col items-center justify-center gap-4 px-8 text-center">
          <svg
            viewBox="0 0 180 140"
            className="size-40 text-primary/25"
            role="img"
  ...
  return (
    <main className="min-h-screen bg-background pb-48" dir="rtl">
      ...
      <div className="mx-auto flex max-w-3xl flex-col gap-4 px-4 py-4">
        ...
        <div className="flex items-center gap-2 rounded-2xl bg-primary/10 px-3 py-3 text-xs font-bold text-primary">
...
{cart.map((item) => {
        ...
        <section className="rounded-2xl border border-border/70 bg-card p-4">
          ...
          {couponMessage && (
            <p
              className={`mt-2 text-xs font-bold ${couponApplied ? "text-primary" : "text-destructive"}`}
            >
              {couponMessage}
            </p>
          )}
      ...
      {/* Sticky CTA */}
      <div className="fixed bottom-16 left-0 right-0 z-30 border-t bg-background/95 p-3 shadow-2xl backdrop-blur">
        <div className="mx-auto flex max-w-3xl items-center gap-3">
          <div className="flex-1">
            <p className="text-xs text-muted-foreground">الإجمالي النهائي</p>
            <p className="text-lg font-black text-primary">{grandTotal} ج.م</p>
          </div>
          <Button
            onClick={onCheckout}
