...
import { updateNavLinkSchema } from '@/lib/validations'
...
export async function PATCH(
  ...
  try {
    const row = await updateNavLink(auth.user, id, parsed.data)
    return ok(row)
  } catch (err) {
...
export async function DELETE(
  ...
  const { id } = await params
  try {
    await deleteNavLink(auth.user, id)
    return ok({ success: true })
  } catch (err) {
    return validationError({ navLink: [(err as Error).message ?? 'تعذر حذف الرابط'] })
  }
}
